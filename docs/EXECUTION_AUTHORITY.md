...
| `EQR_WEB_FILL_GAPS` | `false` | Proposes facts for a thin section via web search, from allow-listed domains only | A second, independent web-search call must confirm each fact before it is written to the Evidence Store; an unconfirmed fact is dropped, never trusted |
